...
export class ContractRepository {
    ...
    private readonly basescanUrl = 'https://api.basescan.org/api?module=contract'

    async getAbi(address: string):Promise<IResponse> {
        const key = "M4X5S8W419727AVR1BTG468ZEJQZQN93SH"

        const url = `${this.basescanUrl}&action=getabi&address=${address}&apikey=${key}`;
    ...
    async getSourceCode(address: string):Promise<IResponse> {
        ...
        const url = `${this.basescanUrl}&action=getsourcecode&address=${address}&apikey=${key}`;
